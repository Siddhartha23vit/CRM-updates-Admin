...
@Component({
  selector: 'app-admin-add-site-visit',
  templateUrl: './admin-add-site-visit.component.html',
  styleUrls: ['./admin-add-site-visit.component.css']
})
export class AdminAddSiteVisitComponent  implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(params => {
      if (params['lead']) {
        ...
        this.newsiteVisit = { ...this.newsiteVisit, ...leadData };
  ...
  updatesitevisit() {
    this.http.put<any>('http://127.0.0.1:5001/api/leads/update-leads', this.newsiteVisit)
      .subscribe(
        response => {
...
deleteProfile() {
  ...
  addsitevisit() {
    this.http.post<any>('http://127.0.0.1:5001/api/site-visits/create-siteVists', this.newsiteVisit)
      .subscribe(
        response => {
          console.log('Site Visit saved successfully:', response);
